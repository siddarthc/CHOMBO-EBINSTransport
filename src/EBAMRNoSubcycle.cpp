...
//  m_scalTrans.resize(nlevels, NULL);
  ...
  m_extraSolverDefined = false;
}
/**********/
void EBAMRNoSubcycle::
defineExtraSolver(const EBAMRLevelFactory* const a_amrLevelFact)
{
  RealVect dxLev;

  // Create hierarchy of levels
  m_amrlevels.resize(m_params.m_maxLevel+1,NULL);

  // Create base level
  m_amrlevels[0] = a_amrLevelFact->new_amrlevel();
  dxLev = m_dx[0]*RealVect::Unit;
  m_amrlevels[0]->assignDx(dxLev);
  m_amrlevels[0]->assignDBLPtr(&m_grids[0]);
  m_amrlevels[0]->assignEBLGPtr(&m_eblg[0]);
  m_amrlevels[0]->assignQuadCFIPtr(&m_quadCFI[0]);
  m_amrlevels[0]->assignAdvVelPtr(m_advVel[0], m_coveredAdvVelLo[0], m_coveredAdvVelHi[0]);
  m_amrlevels[0]->dtPtr(&m_dt);
  m_amrlevels[0]->timePtr(&m_time);
//  m_amrlevels[0]->assignEBISLPtr(&m_ebisl[0]);
  m_amrlevels[0]->define(NULL, m_domain[0], 0, m_params.m_refRatio[0]);

  // Create finer levels
  int ref_factor = 1;
  for (int level = 0; level < m_params.m_maxLevel; ++level)
    {
      dxLev = m_dx[level+1]*RealVect::Unit;
      m_amrlevels[level+1] = a_amrLevelFact->new_amrlevel();
      m_amrlevels[level+1]->assignDx(dxLev);
//      m_amrlevels[level+1]->assignDBLPtr(&m_grids[level+1]);
      m_amrlevels[level+1]->assignEBLGPtr(&m_eblg[level+1]);
      m_amrlevels[level+1]->assignQuadCFIPtr(&m_quadCFI[level+1]);
      m_amrlevels[level+1]->assignAdvVelPtr(m_advVel[level+1], m_coveredAdvVelLo[level+1], m_coveredAdvVelHi[level+1]);
//      m_amrlevels[level+1]->assignEBISLPtr(&m_ebisl[level+1]);
      m_amrlevels[level+1]->dtPtr(&m_dt);
      m_amrlevels[level+1]->timePtr(&m_time);   
      m_amrlevels[level+1]->define(m_amrlevels[level], m_domain[level+1], level+1, m_params.m_refRatio[level+1]);
      m_amrlevels[level]->finerLevelPtr(m_amrlevels[level+1]);
    }

  m_extraSolverDefined = true;
}
/**********/
void
EBAMRNoSubcycle::
...
//      m_scalTrans[ilev] = new LevelData<EBCellFAB>();
...
setupForAMRRun()
{
  ...
  // the extra solver that is being driven doesnt tag cells
  // so I need to initialize extra solver grid only with the new_boxes
...
// if the tags do depend, this is wrong! 
  if (m_extraSolverDefined)
    {
      (initExtraWithTags) ? initialExtraSolverGrid(new_boxes) : initialExtraSolverGrid(old_boxes);
    }
...
//      writePlotFile();
      ...
      if (m_extraSolverDefined) computeExtraSolverInitialDt();
    ...
    {
      m_dt = computeDt();
      if (m_extraSolverDefined) computeExtraSolverDt();
      if ((m_time + m_dt) > a_maxTime)
      {
        m_dt = a_maxTime - m_time;
      }
      ...
      if (m_extraSolverDefined) 
        {
          for (int level = 0; level <= m_params.m_maxLevel; level++)
            {
              m_amrlevels[level]->setNormalVelOld(*m_advVel[level],*m_coveredAdvVelLo[level],*m_coveredAdvVelHi[level]);
              m_amrlevels[level]->setTimeOld(m_dt, m_time);
            }
        }
...
EBAMRNoSubcycle::regrid()
{
  ...
  //and also set EBLGOld in the classes driven by this
  preRegrid();
...
/*********************/
void
EBAMRNoSubcycle::postRegridExtraSolver(int a_base_level)
{
  CH_assert(m_extraSolverDefined);
  ...
  for (int level = m_finestLevel; level >= a_base_level; --level)
    {
...
/*********************/
void
EBAMRNoSubcycle::regridExtraSolver(int a_base_level, const Vector<Vector<Box> >& a_new_grids)
{
  CH_assert(m_extraSolverDefined);
  
//  for (int level = a_base_level + 1; level <= m_finestLevel; ++level)
  for (int level = a_base_level; level <= m_finestLevel; ++level)
    {
      m_amrlevels[level]->m_newEBLGSet = true;
      m_amrlevels[level]->regrid(a_new_grids[level]);
    }

  for (int level = m_finestLevel + 1; level <= m_params.m_maxLevel; ++level)
    {
      m_amrlevels[level]->m_newEBLGSet = true;
...
/*********************/
void
EBAMRNoSubcycle::preRegridExtraSolver(int a_base_level, const Vector<Vector<Box> >& a_new_grids)
{
  CH_assert(m_extraSolverDefined);
  ...
  for (int level = m_finestLevel; level >= a_base_level; --level)
    {
      m_amrlevels[level]->preRegrid(a_base_level, a_new_grids);
    }  
}
/*********************/
void
  ...
  if (m_extraSolverDefined)
    {
      for (int level = 0; level<= m_params.m_maxLevel; ++level)
        {
          m_amrlevels[level]->setEBLGOld(m_eblg[level]);
        }
    }
...
/*********************/
void
EBAMRNoSubcycle::initialExtraSolverGrid(const Vector<Vector<Box> >& a_vectBoxes)
{
  CH_assert(m_extraSolverDefined == true);
/* 
  if (m_extraSolverDefined)
    {
      for (int level = 0; level<= m_params.m_maxLevel; ++level)
        {
          CH_assert(m_amrlevels[level]->m_isEBLGSet);
        }
    }
*/
  ...
  for (int level = 0; level <= m_finestLevel; ++level)
    {
      m_amrlevels[level]->initialGrid(a_vectBoxes[level]);
    }

  for (int level = m_finestLevel; level >= 0; --level)
    {
...
/*
  ...
  for (int ilev = startLevel; ilev <= m_finestLevel; ilev++)
    {
      EBCellFactory ebcellfact(m_ebisl[ilev]);
      m_scalTrans[ilev]->define(m_grids[ilev], 1,  IntVect::Unit, ebcellfact);
    }
  EBAMRDataOps::setToZero(m_scalTrans); 
*/
}
/*********************/
void
...
/*********************/
void
EBAMRNoSubcycle::initialExtraData()
{
/*
  CH_TIME("EBAMRNoSubcycle::initialExtraData");
  if (m_params.m_verbosity >= 3)
    {
      pout () << "EBAMRNoSubcycle::initialExtraData "  << endl;
    }

  for (int ilev = 0; ilev<= m_finestLevel; ilev++)
    {
      RealVect dxLev = m_dx[ilev]*RealVect::Unit;
      m_ibc->initializeScalTrans(*m_scalTrans[ilev], m_grids[ilev], m_ebisl[ilev], m_domain[ilev], RealVect::Zero, m_time, dxLev); 
    }
*/
}
/*********************/
...
/*********************/
void
EBAMRNoSubcycle::computeExtraSolverInitialDt()
{
  CH_assert(m_extraSolverDefined);

  for (int level = 0; level <= m_params.m_maxLevel; ++level)
    {
      // this is just to set m_dt in the amrlevel
      int dtLev = m_amrlevels[level]->computeInitialDt();
    }
}
/*********************/
void
EBAMRNoSubcycle::computeExtraSolverDt()
{
  CH_assert(m_extraSolverDefined);
  ...
  for (int level = 0; level <= m_params.m_maxLevel; ++level)
    {
      // this is just to set m_dt in the amrlevel 
      int dtLev = m_amrlevels[level]->computeDt();
    }
}
/*********************/
void
...
regrid(const Vector<Vector<Box> >& a_newGrids)
{
  ...
  interpolateExtraTerms(1,m_finestLevel);
  uncacheExtraTerms(m_finestLevel);
}
/*****************/
void
EBAMRNoSubcycle::interpolateExtraTerms(int a_startLev, int a_endLev)
{
/*
  Interval interv (0,0);
  m_tempScalTransLDPtr[0]->copyTo(interv, *m_scalTrans[0], interv);
  for (int ilev=a_startLev; ilev<= a_endLev; ilev++)
    {
      //interpolate everywhere
      EBPWLFineInterp ebInterpVec(m_grids[ ilev  ],
                                  m_grids[ ilev-1],
      ...
      ebInterpVec.interpolate(*m_scalTrans[ilev  ],
                              *m_scalTrans[ilev-1],
                              interv);
      m_tempScalTransLDPtr[ilev]->copyTo(interv, *m_scalTrans[ilev], interv);
    } //end loop over levels 
*/
}
/*****************/
void
EBAMRNoSubcycle::uncacheExtraTerms(int a_endLev)
{
/*
  for (int ilev=0; ilev<= a_endLev; ilev++)
    {
      delete m_tempScalTransLDPtr[ilev];
    }
*/ 
}
/*****************/
...
EBAMRNoSubcycle::postTimeStep()
{
  ...
  if (m_extraSolverDefined && !m_advanceGphiOnly) 
    {
      advanceExtraSolver();
      postTimeStepExtraSolver();
    }
}
/*****************/
void
EBAMRNoSubcycle::postTimeStepExtraSolver()
{
...
/*****************/
void
EBAMRNoSubcycle::averageDownExtraTerms()
{
/*
  CH_TIME("EBAMRNoSubcycle::averageDownExtraTerms");
  if (m_params.m_verbosity > 3)
    {
      pout() << "EBAMRNoSubcycle::averageDownExtraTerms" << endl;
    }

  // need to reflux first
  // doing only explicit refluxing for now 
  // do implicit reflux if there is diffusion
  
  // refluxing explicitly
*/  
}
/*****************/
void
...
*/
  ...
  // for the case of transport, th extraSolver needs advVel which gets updated
  //     in the normalVelocityPredictor step in the predictor and remains
  //     unchanged for the timeStep
  //      
  // So I can advance the transport solver after both the predictor
  //     corrector
  //
  // If I want to do subcycling, then I need to update the scalar obtained
  //    by solving transport equation and then use it for refluxing and stuff..

//  if (m_extraSolverDefined && !(m_advanceGphiOnly)) advanceExtraSolver();            
  pointsUpdated();
...
/******************/
void
EBAMRNoSubcycle::advanceExtraSolver()
{
  int level = 0;
  int stepsLeft = 0;
  bool timeBoundary = true;
  (void)timeStepExtraSolver(level,stepsLeft,timeBoundary);
}
/******************/
int
EBAMRNoSubcycle::timeStepExtraSolver(int a_level, int a_stepsLeft, bool a_coarseTimeBoundary)
{
  //subcycling is off so things are quite a bit simpler
  if (a_level != 0)
    {
      MayDay::Error("EBAMRNoSubcycle::timeStepExtraSolver is confused by being called for a_level != 0 with subcycling off");
    }

   {
     ...
     for (int ilev = 0; ilev <= m_finestLevel; ilev++)
       {
         // dt check
         CH_assert(m_dt == m_amrlevels[ilev]->dt());
...
/*****************/
int EBAMRNoSubcycle::getNumExtraVars()
{
  if(m_extraSolverDefined) 
    {
      return m_amrlevels[0]->numConserved();
    }
  else
    {
      return 0;
...
/*****************/
Vector<string> EBAMRNoSubcycle::extraNames()
{
  if(m_extraSolverDefined) 
    {
      return m_amrlevels[0]->getConsNames(); 
    }
  else
    {
      return 0;
...
/*****************/
void EBAMRNoSubcycle::
 addExtraOutputData(LevelData<EBCellFAB>& a_outputData, const int a_startDstInterv, const int a_ilev)
{
  if (m_extraSolverDefined)
    {
      int nComp = getNumExtraVars();
      Interval srcInterv(nComp-1 , nComp-1);
      Interval dstInterv(a_startDstInterv, a_startDstInterv+nComp-1);

      (m_amrlevels[a_ilev]->getConsStateNew()).copyTo(srcInterv, a_outputData, dstInterv); 
    }
}
/*****************/
#ifdef CH_USE_HDF5
...
setupForRestart(const string& a_restartFile)
{
  ...
  if (m_extraSolverDefined)
    {
      setupExtraSolverForRestart(a_restartFile);
    }
}
/*****************/
void
EBAMRNoSubcycle::setupExtraSolverForRestart(const string& a_restartFile)
{
  CH_assert(m_extraSolverDefined);

#ifdef CH_USE_HDF5
  HDF5Handle handle(a_restartFile,HDF5Handle::OPEN_RDONLY);
...
//
  ...
  if (max_level_check != m_params.m_maxLevel)
    {
      pout() << "AMR::restart: checkpoint file inconsistent with inputs to define " << endl;
      pout() << "max level input to define = " << m_params.m_maxLevel << endl;
  ...
  if (m_params.m_verbosity >= 2)
    {
      pout() << "read max_level = " << m_params.m_maxLevel << endl;
    }
  ...
  CH_assert(m_finestLevel == num_levels - 1);
  int finestLevelOld = m_finestLevel;

  if (m_finestLevel > m_params.m_maxLevel)
    {
      pout() << "AMR::restart: checkpoint file inconsistent with inputs to define " << endl;
      pout() << "numlevels input to define = " << m_params.m_maxLevel + 1<< endl;
  ...
  if (m_params.m_verbosity >= 2)
    {
      pout() << "set finest_level = " << m_finestLevel << endl;
    }
  ...
  m_curStep = header.m_int ["iteration"];
  if (m_params.m_verbosity >= 2)
    {
      pout() << "read cur_step = " << m_curStep << endl;
    }
  ...
  CH_assert(m_time == header.m_real["time"]);
  ...
  for (int level = 0; level <= m_finestLevel; ++level)
    {
      ...
      handle.setGroup("/");
      m_amrlevels[level]->readCheckpointHeader(handle);
      m_amrlevels[level]->readCheckpointLevel(handle);
    }

  for (int level = 0; level < m_finestLevel; ++level)
    {
      int refratio_test = m_amrlevels[level]->refRatio();
      if (refratio_test != m_params.m_refRatio[level])
        {
          pout() << "AMR::restart: checkpoint file inconsistent with inputs to define " << endl;
          pout() << "for level " << level << endl;
          pout() << "refratio input to define = " << m_params.m_refRatio[level] << endl;
...
// dt check
  for (int level = 0; level <= m_finestLevel; ++level)
    {
      CH_assert(m_dt = m_amrlevels[level]->dt());
    }
...
void
EBAMRNoSubcycle::
cacheExtraTerms(int a_endLevel)
{
/*
  Interval interv(0, 0);
//  Vector<LevelData<EBCellFAB>* > tempLDPtr;
  m_tempScalTransLDPtr.resize(a_endLevel+1);

  for (int ilev=0; ilev<= a_endLevel; ilev++)
    {
      EBCellFactory ebcellfact(m_ebisl[ilev]);
      m_tempScalTransLDPtr[ilev] = new LevelData<EBCellFAB>();
      m_tempScalTransLDPtr[ilev]->define(m_grids[ilev], 1,  IntVect::Unit, ebcellfact);
      m_scalTrans[ilev]->copyTo(interv, *(m_tempScalTransLDPtr[ilev]), interv);
    }
*/
}
/*****************/
